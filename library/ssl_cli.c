	...
	size_t len)
{
	...
	if (*p != (char)ssl->conf->cid) {
		// if we get here then something got messed up since the 
		// server should not select something that is incompatible 
		// with our settings provided in the client hello.  
		MBEDTLS_SSL_DEBUG_MSG(1, ("bad server hello message"));
		return(MBEDTLS_ERR_SSL_BAD_HS_SERVER_HELLO);
	}
	else {
	...
	// this is the CID the server is expecting for inbound data
	...
	size_t *olen)
{
	...
	*p++ = (unsigned char)(5 & 0xFF);
	...
	// cid value -- either static or dynamic 
	*p++ = (char) ssl->conf->cid;
