...
/*
 * Handler for MBEDTLS_SSL_SERVER_NEW_SESSION_TICKET
 */
                                                    ...
                                                    size_t *out_len )
{
    ...
    *out_len = 0;
    ...
    session->ciphersuite = ssl->handshake->ciphersuite_info->id;
    ...
    MBEDTLS_SSL_CHK_BUF_PTR( p, end,
        4 + 4 + 1 + MBEDTLS_SSL_TICKET_NONCE_LENGTH + 2 + 2 );
    ...
    /* Generate ticket_once */
    p[8] = MBEDTLS_SSL_TICKET_NONCE_LENGTH;
    ret = ssl->conf->f_rng( ssl->conf->p_rng, p + 4 + 4 + 1,
    ...
    MBEDTLS_SSL_DEBUG_BUF( 3, "ticket_nonce:",
                           p + 4 + 4 + 1, MBEDTLS_SSL_TICKET_NONCE_LENGTH );
    ...
    MBEDTLS_SSL_DEBUG_BUF( 3, "Ticket-resumed PSK", session->key, hash_length );
    ...
    ret = ssl->conf->f_ticket_write( ssl->conf->p_ticket,
                                     session,
                                     p + 9 + MBEDTLS_SSL_TICKET_NONCE_LENGTH + 2,
                                     end,
                                     &ticket_len,
                                     &ticket_lifetime);
    if( ret != 0 )
    ...
    p += 9 + MBEDTLS_SSL_TICKET_NONCE_LENGTH;
    MBEDTLS_PUT_UINT16_BE( ticket_len, p, 0 );
    p += 2;
    MBEDTLS_SSL_DEBUG_BUF( 4, "ticket->ticket", p, ticket_len);
    p += ticket_len;
...
{
    mbedtls_ssl_handshake_set_state( ssl, MBEDTLS_SSL_NEW_SESSION_TICKET_FLUSH );
    return( 0 );
}
